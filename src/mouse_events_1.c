...
void	sound_press(SDL_MouseButtonEvent b, t_env *env)
{
	...
	{
		if (b.x >= W_W - 60 && b.x <= W_W - 10 && b.y >= 10 && b.y
			<= 50 && env->screen == 2 && env->options_pupop_showed == 0)
		{
			if (Mix_Paused(2) == 1)
			{
				env->frame_sound = 0;
...
void	play_press(SDL_MouseButtonEvent b, t_env *env)
{
	...
	if (b.button == SDL_BUTTON_LEFT)
	{
		if (b.x >= W_W / 2 - 125 && b.x <= W_W / 2 + 125 && b.y
			>= W_H / 3 && b.y <= W_H / 3 + 155 && env->screen == 1)
		{
			Mix_Resume(2);
...
void	quit_press(SDL_MouseButtonEvent b, t_env *env)
{
	if (b.button == SDL_BUTTON_LEFT)
	{
		if (b.x >= W_W / 2 - 125 && b.x <= W_W / 2 + 125 && b.y >= W_H / 3
			+ 200 && b.y <= W_H / 3 + 200 + 155 && env->screen == 1)
		{
...
void	close_btn_press(SDL_MouseButtonEvent b, t_env *env)
{
	if (b.button == SDL_BUTTON_LEFT)
	{
		if (b.x >= W_W - 55 && b.x <= W_W - 15 && b.y >= 15
			&& b.y <= 55 && env->options_pupop_showed == 1)
