...
/*   Updated: 2022/05/20 19:01:40 by amya             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_map_line(char **ar, t_env *env, int start, int line)
{
	int	i;
	...
	while (ar[i])
	{
		if ((i == env->mps - 1 || i == 0 || line == 0 || line == env->mps - 1)
			&& !ft_strcmp(ar[i], "0"))
			error_exit("map must have wall borders");
...
void	parsing(t_env *env, char *file)
{
	...
	while (get_next_line(var.fd, &var.line) > 0)
	{
		...
		fill_map_line(var.ar, env, var.elems * env->mps, var.elems);
		var.elems++;
	}
	close(var.fd);
